...
class Mouse:
    def __init__(self,mouse_image1, mouse_image2):
...
class Button():
    ...
    def draw(self,outline=None,action=None, colorChange=True):
        ...
        if self.buttonType == "Circle":
            ...
            if outline:
                """draws an outline"""
                pygame.draw.circle(self.screen, outline, (self.x, self.y),self.radius+4,0)
            """Trig for area"""
            ...
            if difference <= self.radius:
                """Draws a lighter version of the image"""
                pygame.draw.circle(self.screen, self.color2, (self.x,self.y),self.radius,0)
